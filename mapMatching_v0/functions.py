...
def str2datetime(strTime):
    ...
    fin=datetime.now()
    tiempo_ejecucion=fin-inicio
    print( "Time function(  ): " + str( tiempo_ejecucion ) )
...
def near_segments(index,tempData,roadway,tempTable,searchRadius,gpsDict,snapDict):
    ...
    # fin=datetime.now()
    # tiempo_ejecucion=fin-inicio
    # functionexecution(fin-inicio): " + str( tiempo_ejecucion ) )   
    # return snapDict
                ...
                if fid_i > 0:
                    ...
                    for street in snapDict[j]:
                        ...
                        if snapSpeed - tol_rs <= avSpeed <= snapSpeed + tol_rs:   
                            ...
                            print "FID = i: " +str(fid_i)+ "  &   j: " +str(fid_j)
                            fin=datetime.now()
                            tiempo_ejecucion=fin-inicio
        ...
        if solution:
            ...
            if j - i > 1:
                ...
                for oid in range(i + 1,j):
                    ...
                    upCursor.updateRow(row)
                    del upCursor
                    #TODO: validar roadway entre comillas
...
def compareFID(n,finalData,gpsDict):
    ...
    countAnalysis() # View count access method
    fin=datetime.now()
    tiempo_ejecucion=fin-inicio
